...
public class Wordle {
	...
	public static void main(String[] args) {
		...
		int[] match;
		int contadorPalabras = 20, intentos = 0, partidasJugadas = 0, partidasGanadas = 0, max;
		boolean wrong;
		...
		do {
			...
			do {
				...
				do {
					...
					for (int i = 0; i<nLetras; i++) {
						...
						if (resultado.charAt(i) == '2') {
							if (presente(letras0, a) || (palabra2T[i]!=a && palabra2T[i]!='0')) {
								wrong = true;
								msgWrong = "La entrada no es coherente. Vuelva a introducirla (" + (i+1) + ").";
							} else {
				...
				intentos++;
			} while (!resultado.equals(correcto) && intentos<intentosMax);
